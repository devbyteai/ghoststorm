...
class TestFlowRecordPlaybackJourney:
    ...
    def test_complete_flow_workflow(
        ...
        with patch("ghoststorm.api.routes.flows.get_flow_recorder", return_value=mock_recorder):
            with patch("ghoststorm.api.routes.flows.get_flow_storage", return_value=mock_storage):
        ...
        with patch("ghoststorm.api.routes.flows.get_flow_storage", return_value=mock_storage):
            with patch("ghoststorm.api.routes.flows.get_flow_executor", return_value=mock_executor):
    ...
    def test_execution_status(self, api_test_client: TestClient):
        ...
        with patch("ghoststorm.api.routes.flows.get_flow_executor", return_value=mock_executor):
            ...
            with patch(
                ...
                response = api_test_client.get(f"/api/flows/executions/{mock_result.execution_id}")
